...
path_to_file = pathlib.Path("data/fra.txt")
...
fr_sentences, eng_sentences = load_data(path_to_file)

sentences = (fr_sentences, eng_sentences)

BUFFER_SIZE = len(eng_sentences)
BATCH_SIZE = 64

is_train = np.random.uniform(size=(len(fr_sentences),)) < 0.8

train_raw = (
    tf.data.Dataset.from_tensor_slices(
        (eng_sentences[is_train], fr_sentences[is_train])
    )
    .shuffle(BUFFER_SIZE)
    .batch(BATCH_SIZE)
)
...
french_vectorizer.adapt(train_raw.map(lambda context, target: target))


def process_text(context, target):
    context = english_vectorizer(context).to_tensor()
    target = french_vectorizer(target)
